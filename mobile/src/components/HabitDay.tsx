...
export function HabitDay({ amountOfHabit = 0, amountCompleted = 0, date, ...rest }: HabitDayProps) {
    ...
    return (
        <TouchableOpacity className={clsx("rounded-lg border-2 m-1", {
            ...
            ["bg-violet-900 border-violet-700"]: amountAccomplishedPercentage > 
            0 && amountAccomplishedPercentage < 20,
            ...
            ["border-violet-400 border-3"]: isCurrentDay,
        })}
            style={{ width: DAY_SIZE, height: DAY_SIZE }} activeOpacity={.7}
            {...rest}
        >
